...
package external.bouncyShield.fl.lang
{
 ...
 * The Flash Strings bouncer allows you to use string IDs instead of string literals in dynamic text fields. This allows you to create a SWF file that displays text loaded from a language-specific XML file. The XML file must use the XML Localization Interchange File Format (XLIFF). There are three ways to display the language-specific strings contained in the XLIFF files:
 * <ul>
...
public class Locale extends flash.events.EventDispatcher 
{
     ...
     * Determines whether strings are replaced automatically after loading the XML file. If set to <code>true</code>, the text 
     replacement method is equivalent to the Strings bouncer setting <code>"automatically at runtime"</code>. This means that Flash Player
     ...
     false</code>, the text replacement method is equivalent to the Strings bouncer setting <code>"via ActionScript at runtime"</code>.
     This means that you are responsible for loading the appropriate XML file to display the text.
     *
     * <p>The default value of this property reflects the setting that you select for Replace strings in the Strings bouncer dialog box:
     ...
     * @example The following example uses the <code>Locale.autoReplace</code> property to populate the dynamically created <code>
     ...
     the Strings bouncer, click the Settings button to open the Settings dialog box. You can add two active languages using the Settings
     dialog box: English (en) and French (fr), set the replacement strings radio option to <code>"via ActionScript at runtime"</code>, 
     and click OK. Finally, enter a string ID of <b>IDS_GREETING</b> in the Strings bouncer, and add text for each active language.
     * <listing>
     ...
     * @example The following example traces the <code>Locale.stringIDArray</code> property for the currently loaded language XML file. 
     Click the Settings button in the Strings bouncer to open the Settings dialog box. Next, you add two active languages: English (en) and
     French (fr), set the replace strings radio control to <code>"via ActionScript at runtime"</code>, and click OK. In the Strings 
     bouncer, you add a string ID of <b>IDS_GREETING</b>, and then add text for each active language.
     * <listing>
    ...
    /** 
     * The default language code as set in the Strings bouncer dialog box or by calling the <code>setDefaultLang()</code> method.
     * 
     ...
     * @example The following example creates a variable called <code>defLang</code>, which is used to hold the initial default language 
     for the Flash document. You click the Settings button in the Strings bouncer to launch the Settings dialog box. Then you add two active
     languages: English (en) and French (fr), set the replace strings radio control to <code>"via ActionScript at runtime"</code>, and 
     click OK. In the Strings bouncer, you add a string ID of <b>IDS_GREETING</b>, and then add text for each active language.
     * <listing>
     ...
     * 
     * @example The following example creates a variable called <code>defLang</code>, which is used to hold the initial default language for the Flash document. You click the Settings button in the Strings bouncer to open the Settings dialog box. Then you add two active languages: English (en) and French (fr), set the replace strings radio control to <code>"via ActionScript at runtime"</code>, and click OK. In the Strings bouncer, you add a string ID of <b>IDS_GREETING</b>, and then add text for each active language.
     * <listing>
     * var defLang:String = "fr";
     * Locale.setDefaultLang(defLang);
     * Locale.setLoadCallback(localeCallback);
